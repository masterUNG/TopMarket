...
class ClientProductUnitListView extends StatefulWidget {
//String itemID, String itemName
  ClientProductUnitListView(
...
//-- ตัวแปรลูก
  ...
  @override
  _ClientProductUnitListViewState createState() =>
      _ClientProductUnitListViewState();
}

class _ClientProductUnitListViewState extends State<ClientProductUnitListView> {
  Timer? timer;
        ...
        if (this.masterModelList.length > 0) {
          ...
          return ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                MasterModel item = this.masterModelList[index];
                return buildModelDataList(item, context);
              });
        } else {
          return LinearProgressIndicator(
              backgroundColor: Colors.brown[100], color: Colors.brown[200]);
        }
      },
    );
  }

  Container buildModelDataList(MasterModel model, BuildContext context) {
    return Container(
        ...
        child: Card(
          child: Row(
            children: [
              ...
              //       "/asset/get_image_unit.php?client=" +
  ...
  Future<List<MasterModel>> getMasterModelList() async {
    String url = MyConstant.apiDomainName.toString() +
        "/lookup/product_unit.php?client=" +
        MyConstant.currentClientID.toString() +
        "&key=" +
    ...
    Dio().get(url).then((value) {
      ...
      for (var item in items) {
        this.masterModelList.add(MasterModel.fromMap(item));
